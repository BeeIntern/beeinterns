...
describe('Test Beeline Shop', () => {
    ...
    it('Первый тест', () => {
        ...
        addStep("Выбираем продукцию Samsung");
        $("//input[@type='checkbox'][contains(@name, 'samsung')]").click();
        addStep("Проверяем, что отображается продукция Samsung");
        browser.waitUntil(() => {
            let titles = $$("//div[contains(@class,'ProductCard_header')]/a");
            for (let i = 0; i < titles.length; i++) {
                if (titles[i].getText().includes('Samsung')) {
                    return expect(titles[i].getText()).to.contains('Samsung');
                };
                return expect(titles[i].getText()).to.contains('Samsung');
            }
