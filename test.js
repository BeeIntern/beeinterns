...
describe('Test Beeline Shop', function() {
    ...
    it("Один из чекбоксов производителей, например, \"Samsung\”", () => {
        $("//input[@type='checkbox'][contains(@name, 'samsung')]").click();
        browser.waitUntil(() => {
            let title = $("h1[class*='Heading_h1']").getText();
            return expect(title).to.contains('Samsung');
        }, 20000, 'expected h1 contains "Samsung"');
    })
    ...
    it("Элемент наименования товара в каталоге", () => {
        ...
        expect(name).to.contains('Samsung');
    })
    ...
    it("Кнопка 'Купить'", () => {
        ...
        buyButton.waitForClickable(20000);
        buyButton.click();
    ...
    it("Кнопка восстановления удалённого товара", () => {
        ...
        }, 15000, "expects button 'Buy' is will work successfully");
    })
